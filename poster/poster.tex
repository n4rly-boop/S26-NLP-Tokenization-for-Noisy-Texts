...
\item Single component dominates per channel (truecase / ASR, charfix / OCR) \,---\, target, don't uniformly preprocess.
...
Byte-level is structurally noise-robust (\callout{+4\%}). Preprocess restores
...
truecasing recovers \callout{half}. ByT5 stays above 0.67 across all channels.
}
...
\raggedright\small
\textbf{No universal winner.} Preprocess dominates gpt2; noisy-fine-tune
dominates bert-uncased and byt5; the two \emph{tie} on bert-cased. ASR rows are
...
\footnotesize
\textbf{Limits:} 1-epoch training; rule-based preprocess (no contextual MLM); punct-restore out-of-scope (BIO); English NER only. Refs (Belinkov\&Bisk, TextFlint, CoNLL, seqeval) in repo README.\\[1pt]
